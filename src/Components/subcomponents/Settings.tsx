...
const ColorBox: FC<colorProps> = ({ colorCode, overlayColor }): JSX.Element => {
  let className = classNames({
    'color-box ': true,
    'selected-color inner-shadow': colorCode === overlayColor,
  });

  return (
    <div
      className={className}
      style={colorBoxStyle(colorCode === overlayColor, colorCode)}
    ></div>
  );
};
...
const Settings = () => {
  ...
  return (
    <div className="settings flex flex-c  h-100" style={{ padding: '15px' }}>
      <div className="title-area flex flex-r tertiary-color">
        <div>Settings</div>
      </div>
      <div className="overlay-card flex flex-c other-gray-2">
        <div className="title flex flex-c w-100 other-color-2">
          <div>Overlay Color</div>
          <div className="option-subtitle">
            Set the color of the positive capture overlay
          </div>
        </div>
        <Grid container className="color-options flex flex-r" style={{}}>
          {colorArray.map((color, index) => {
            return (
              ...
              >
                <ColorBox overlayColor={overlayColor} colorCode={color} />
              </Grid>
            );
          })}
        </Grid>
      ...
      <div className="control-scheme flex flex-c other-gray-2">
        ...
        <div className="options-area flex flex-c other-color-2">
          {/* <div className="option flex flex-r align-center-h">
            <div className="left flex flex-c">
              <div>Filter Low Certainty Predictions</div>
              <div className="option-subtitle">
                Show Positive Predictions below 70%
              </div>
            </div>
            <div className="right">
              <IOSSwitch
                checked={filterHighCertaintyPredictions}
                onChange={(e: any) => {
                  setfilterHighCertaintyPredictions(e.target.checked);
                }}
              />
            </div>
          </div>
          <div className="option flex flex-r align-center-h">
            <div className="left flex flex-c">
              <div>Filter High Certainty Predictions</div>
              <div className="option-subtitle">
                Show Positive Predictions above 70%
              </div>
            </div>
            <div className="right">
              <IOSSwitch
                checked={filterLowCertaintyPredictions}
                onChange={(e: any) => {
                  setfilterLowCertaintyPredictions(e.target.checked);
                }}
              />
            </div>
          </div> */}
          ...
          <div className="option flex flex-r align-center-h">
            <div className="left flex flex-c">
              <div>Overlays Visible</div>
              <div className="option-subtitle">
                Show/Hide Positive Capture Overlays
              </div>
            </div>
            <div className="right">
              <IOSSwitch
                ...
                onChange={(e: any) => {
                  setoverlaysVisible(e.target.checked);
                }}
              />
            </div>
          </div>
      ...
      {/* <div className="other-settings flex flex-c other-gray-2">
        ...
        <div className="options-area flex flex-c other-color-2">
          <div className="option flex flex-r align-center-h">
            <div className="left">Unit of Measurement</div>
            <div className="right flex flex-r">
              <div
                ...
                className={classNames({
                  'unit-btn tertiary-bg': unit === 'metric',
                  'unit-btn secondary-bg other-color-2': unit !== 'metric',
                })}
              >
                M
              </div>
              <div
                ...
                className={classNames({
                  'unit-btn tertiary-bg': unit === 'feet',
                  'unit-btn secondary-bg other-color-2': unit !== 'feet',
                })}
              >
                F
              </div>
